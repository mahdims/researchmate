...
### [FLARE: Verifying MILP Reformulations with LLM-Based Theorem Proving](https://arxiv.org/abs/2608.25220)

**2026-08-25** | Stanford University | M=8 P=9 I=9 **MUST-READ** *changes-thinking* *discuss*

*Method:* LLM-based agent combined with Lean proof assistant for autoformalization and automated theorem proving (ATP) | *LLM role:* autoformalization, theorem_proving_agent

> Robbins et al. introduce FLARE, an automated framework that uses an LLM agent and the Lean proof assistant to formally verify whether one MILP formulation is a valid reformulation of another. The authors demonstrate 100% accuracy on a new benchmark of 54 NP-hard formulation pairs, backed by machine-checkable proofs rather than heuristic execution checks. The key insight is that instance-level validation—commonly used to evaluate LLM-generated models—is fundamentally flawed and can accept invalid transformations; the authors prove this by identifying mathematically invalid cutting planes generated by recent LLM evolutionary search frameworks. This is a critical read for our work in automated optimization modeling and LLM-driven heuristic design, as it provides a concrete, verifiable method (and a cheaper natural-language proxy, FLARE-NL) to ensure generated formulations and cuts are universally valid across all problem instances.
